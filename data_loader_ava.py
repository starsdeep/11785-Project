...
class ClipBatchIter(mx.io.DataIter):
    def __init__(self, config, train):
        ...
        # exclude_path = config.exclude_train if train else config.exclude_test
        # with open(exclude_path, 'rb') as fd:
        #     exclude = pickle.load(fd)
        # print('exclude', exclude)


        self.grouped = list(self.df.groupby(['video_id', 'timestamp', 'person_id']))
        # print('len grouped before exclude', len(self.grouped))
        # self.grouped = [(index, df) for index, df in self.grouped if (index[0], index[1]) not in exclude]
        # print('len grouped after exclude', len(self.grouped))
    ...
    def get_bbox_and_label(self, grouped_sample, flip, p, scale_w, scale_h):
        ...
        df = grouped_sample[1]
        # currently fix self.n_bbox to 1
        cnt = 0
