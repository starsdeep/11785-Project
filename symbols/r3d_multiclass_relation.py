...
import logging
import numpy as np
from attention import relation_feature
...
def create_r3d(
    ...
    body_relation = relation_feature(body, rois, batch_per_device, nongt_dim, geometric_dim)
    body_relation = mx.sym.reshape_like(lhs=body_relation, rhs=body)
    body = body + body_relation
    ...
    # apply mask
    mask = mx.sym.var('mask', dtype=np.float32)
    mask = mx.sym.reshape(data=mask, shape=(-1, 1))
    body = mx.sym.broadcast_mul(lhs=body, rhs=mask, name="masked_body")
    label = mx.sym.broadcast_mul(lhs=label, rhs=mask, name="masked_label")
